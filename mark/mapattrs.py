...
классов; различные словарные инструменты, использующие включения Python З.Х/2.7.
"""
import pprint
...
invertdict(diсt(а=1,  b=2,  с=1)) =>  {1: ['а’, 'с'], 2:   ['Ь'])
    """
    def keysof(V):
...
bysource:  False=сгруппировать результат по  объектам,  а  не  атрибутам.
...
if __name__ == '__main__':
    print('Classic classes in 2.X, new-style in З.Х')
    class A: attr1 = 1
    class B(A): attr2 = 2
    class C(A): attr1 = 3
    class D(B, C): pass
    ...
    print('New-style classess in 2.X and 3.X')
